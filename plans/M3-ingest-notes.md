...
- **Upsert must preserve notes.** For each page 1..N: find-or-initialize
  `Slide` by `(lesson_id, page_number)`, replace the attached `image`, keep
  existing `notes` untouched. Delete slides with `page_number > N`. Wrap in a
  transaction.
...
- One row per slide: thumbnail, page number, `<textarea>` for `notes`.
- **Autosave on blur**: each row is its own small form; a ~10-line Stimulus
  controller submits via `requestSubmit()` on textarea blur; respond with a Turbo
  Stream that flashes a "Saved" indicator on the row.
...
- **Re-upload preserves notes**: ingest deck A (3 pages), type notes on page 2,
  ingest corrected deck A′ (3 pages) → notes survive; ingest 2-page deck → slide 3
  deleted.
- Import task round-trip on a fixture folder: creates the lesson, applies notes,
  idempotent on second run; malformed front matter aborts cleanly.
